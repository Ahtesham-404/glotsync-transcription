...
export function LoginPage() {
  ...
  const onSubmit = async (values: LoginForm) => {
    setAuthError(null)
    try {
      await signInWithEmail(values.email, values.password, values.rememberMe ?? true)
        ...
        <div className="glass rounded-2xl p-8 shadow-2xl">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4" noValidate>
            ...
            <label className="flex items-center gap-2.5 cursor-pointer">
              <input
                type="checkbox"
                className="w-4 h-4 rounded border-surface-500 bg-surface-800 text-brand-500 focus:ring-brand-500 focus:ring-offset-surface-900"
                {...register('rememberMe')}
              />
